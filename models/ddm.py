...
class Unet(Module):
    ...
    def forward(self, x, time, x_self_cond = None):
        ...
        if self.self_condition or self.condition:
...
class GaussianDiffusion(Module):
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, return_all_timesteps = False, x_cond=None):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc = 'sampling loop time step', total = self.num_timesteps):
            ...
            img, x_start = self.p_sample(img, t, x_cond) 
            imgs.append(img)
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, return_all_timesteps = False, x_cond=None, b=1):
    ...
    @torch.no_grad()
    def sample(self, batch_size = 16, return_all_timesteps = False, x_cond=None,b=1):
        _, _, h, w = x_cond.shape
        channels = self.channels
        # if x_cond!=None:
        #     x_cond = self.normalize(x_cond)
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn((batch_size, channels, h, w), return_all_timesteps = return_all_timesteps, x_cond=x_cond, b=b)
    ...
    def p_losses(self, x_start, x_cond,  t, noise = None, offset_noise_strength = None):
        ...
        # model_out = self.unnormalize(model_out)
    ...
    def forward(self, img, x_cond=None, *args, **kwargs):
        ...
        # assert h == img_size[0] and w == img_size[1], f'height and width of image must be {img_size}'
            self.image_size = (h, w)
            # print('img_size:', self.image_size)
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long()

        # img = self.normalize(img)
        # if x_cond != None:
        #     x_cond = self.normalize(x_cond)
        return self.p_losses(img, x_cond, t, *args, **kwargs)
